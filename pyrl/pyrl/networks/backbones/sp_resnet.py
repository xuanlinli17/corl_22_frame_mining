...
class SparseResNet(SparseCNNBase):
    ...
    def forward(self, points, **kwargs):
        ...
        """
        if self.feature_proj_net is not None:
            mean_xyz = torch.mean(xyz, dim=1) # [B, 3]
            mean_xyz = torch.repeat_interleave(mean_xyz.unsqueeze(1), xyz.shape[1], dim=1)
            pcd_feature_proj = torch.cat([mean_xyz, pcd_feature], dim=-1)   
            pcd_feature_proj = self.feature_proj_net(pcd_feature_proj)
            z_proj = build_points(xyz, pcd_feature_proj)
            x_proj = initial_voxelize(z_proj, 1.0, self.voxel_size)
            z_proj_out = voxel_to_point(x_proj, z_proj, nearest=False)
            z.F = torch.cat([z.F, z_proj_out.F], dim=-1)
        """
        ...
        if self.use_pvcnn:
            ...
            z1.F = z1.F + self.point_transforms(z0.F)
