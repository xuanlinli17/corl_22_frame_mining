...
from .type_utils import get_dtype, is_np, is_np_arr, is_num, is_torch, is_integer, is_torch_distribution, is_not_null, is_arr
from .wrapper import seq_to_np
...
def take(item, indices, axis=None):
    if isinstance(item, list):
        assert axis == 0 and is_num(indices)
        return item[indices]
    # print(indices)
    # exit(0)
    if isinstance(indices, slice):
        if axis == 0:
            return slice_item(item, indices)
        indices = slice_to_range(indices)
    if isinstance(indices, range):
        if axis == 0:
            return slice_item(item, range_to_slice(indices))
        indices = list(indices)
    indices = np.array(indices, dtype=np.int64) if isinstance(indices, (list, tuple)) else indices

    if is_np_arr(item):
        if not is_integer(indices):
...
def share_memory(x, y):
    if type(x) != type(y):
        ...
        return (x.storage().data_ptr() == y.storage().data_ptr()).any()
    else:
