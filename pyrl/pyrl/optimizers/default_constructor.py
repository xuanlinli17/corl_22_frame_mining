...
from pyrl.utils.meta import build_from_cfg
from pyrl.utils.data import is_list_of
...
class DefaultOptimizerConstructor:
    ...
    def _validate_cfg(self):
        ...
        if "custom_keys" in self.paramwise_cfg:
            if not isinstance(self.paramwise_cfg["custom_keys"], dict):
                raise TypeError("If specified, custom_keys must be a dict, " f'but got {type(self.paramwise_cfg["custom_keys"])}')
    ...
    def __call__(self, model):
        ...
        if not self.paramwise_cfg:
            ...
            #     # list of parameters from potentially diff models
            #     assert isinstance(model[0], nn.Parameter)
            #     optimizer_cfg['params'] = model
            else:
