...
def read_images_from_camera(cam, rgb=True, depth=False, seg_indices=None):
    img = {}
    if rgb:
...
def read_pointclouds_from_camera_with_depth_filtering(
    cam, rgb=True, depth=True, seg_indices=None
):
...
def read_pointclouds_from_camera(cam, rgb=True, depth=True, seg_indices=None):
    # the depth parameter will be ignored
    pcd = {}
    pos_depth = get_texture_by_dltensor(cam, "Position")
...
class CombinedCamera(object):
    ...
    def get_images_list(self, rgb=True, depth=False, seg_indices=None):
        view_list = []
        for cam in self.sub_cameras:
            view_dict = read_images_from_camera(
                cam, rgb=rgb, depth=depth, seg_indices=seg_indices
            )
            view_list.append(view_dict)
        return view_list

    def get_fused_pointcloud(self, rgb=True, seg_indices=None):
        pcds = []
        for cam in self.sub_cameras:
            pcd = read_pointclouds_from_camera(
                cam, rgb=rgb, seg_indices=seg_indices
            )  # dict
            pcds.append(pcd)
    ...
    def get_combined_view(self, mode, rgb=True, depth=False, seg_indices=None):
        if mode in ["color_image", "rgb", "rgbd", "depth"]:
            return self.get_images_list(rgb=rgb, depth=depth, seg_indices=seg_indices)
        else:  # pointcloud
            return self.get_fused_pointcloud(rgb=rgb, seg_indices=seg_indices)
