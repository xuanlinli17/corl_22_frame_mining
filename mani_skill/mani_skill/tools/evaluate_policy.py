...
def parse_args():
    ...
    parser.add_argument('--level-list',  type=str, default=None) # format is like 1,3,5,6, this will override level-range
    parser.add_argument('--json-path',  type=str, default=None) # this will override level-list
    parser.add_argument('--json-index-range',  type=str, default=None) # format is like $START_INDEX-$END_INDEX
...
def parse_levels(args):
    if args.json_path is not None:
        ...
        for k, v in d.items():
            levels += v
        if args.json_index_range is not None:
            s = args.json_index_range
            # s is like 100-200
            if '-' not in s:
                raise Exception('Incorrect json index range format, it should be like 100-200.')
            a, b = s.split('-')
            a, b = int(a), int(b)
            levels = levels[a:b]
        return levels
    elif args.level_list is not None:
        s = args.level_list
        levels = s.split(',')
